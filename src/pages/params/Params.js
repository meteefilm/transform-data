...
export default function Params() {
    ...
    const textEditor = (rowData, { column }) => {
        ...
        return <InputText type="text" value={rowData[field]} onChange={(e) => onCellEditComplete(e, field, rowData)} />;
    };

    const dropEditor = (rowData, { column }) => {
        const { field = '' } = column.props;
        return <Dropdown value={rowData[field]} onChange={(e) => onCellEditComplete(e, field, rowData)} options={typeList} />;
    };
                ...
                <div className="p-fluid formgrid grid ">
                    <div className="field col-12 ">
                        <div className="p-inputgroup">
                            {type === 'req' ? <InputText id={e.tags} placeholder="..." value={e.text} /> : <InputTextarea id={e.tags} placeholder="..." value={e.text} rows={3} />}
                            <Button icon="pi pi-copy" onClick={() => onClickCopy(e.text)} />
                        </div>
                    </div>
                </div>
            );
                                    ...
                                    <div className={'field col-12 md:col-6'}>
                                        ...
                                        <DataTable value={transDataList}>
                                            {columns.map(({ field, header }) => {
                                                return <Column key={field} field={field} header={header} style={{ width: '50%' }} body={field === 'type' ? dropEditor : textEditor} />;
                                            })}
                                        </DataTable>
                                    </div>
                                    <div className="field col-12 md:col-6">
                                        ...
                                        <Button
                                            className={classNames(
                                                'p-button-outlined w-3rem mb-2 mr-2 ',
                                                { 'p-button-secondary': transFileView === 'text' },
                                                { 'p-button-success': transFileView === 'list' && transFile !== '' },
                                                { 'p-button-secondary': transFile === '' }
                                            )}
                                            onClick={() => setTransFileView('list')}
                                            icon="pi pi-list"
                                            disabled={transFile === ''}
                                        />
                                        <Button
                                            className={classNames(
                                                'p-button-outlined w-3rem mb-2 mr-2 ',
                                                { 'p-button-secondary': transFileView === 'list' },
                                                { 'p-button-success': transFileView === 'text' && transFile !== '' },
                                                { 'p-button-secondary': transFile === '' }
                                            )}
                                            onClick={() => setTransFileView('text')}
                                            icon="pi pi-code"
                                            disabled={transFile === ''}
                                        />
